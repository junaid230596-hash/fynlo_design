// Fynlo Business service worker — isolated from the admin/individual apps
...
const CACHE_NAME = 'fynlo-business-shell-v2';
  ...
  if (isNavigation) {
    event.respondWith(
      fetch(request)
        ...
        .catch(() =>
          caches.match(request).then((cached) => cached || caches.match('fynlo-business.html'))
        )
    );
    return;
  }
