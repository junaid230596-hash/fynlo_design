// Fynlo Individual service worker — isolated from the admin/business apps
...
const CACHE_NAME = 'fynlo-individual-shell-v2';
const PRECACHE_ASSETS = [
  'fynlo-individual.html',
  'icons/icon-192.png',
  'icons/icon-512.png',
  ...
  if (isNavigation) {
    event.respondWith(
      fetch(request)
        ...
        .catch(() =>
          caches.match(request).then((cached) => cached || caches.match('fynlo-individual.html'))
        )
    );
    return;
  }
